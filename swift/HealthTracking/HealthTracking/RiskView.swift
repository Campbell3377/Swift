...
func displayMessage(for records: [HealthRecord]) -> some View {
    ...
    let latestBloodPressure = latestRecord.bloodPressure
    let previousBloodPressure = previousRecord.bloodPressure
    
    let currBP:[Double] = [Double(latestBloodPressure[0]), Double(latestBloodPressure[0])]
    let prevBP:[Double] = [Double(previousBloodPressure[0]), Double(previousBloodPressure[0])]
...
struct RiskView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        let viewModel = RiskViewModel(healthModel: healthModel)
